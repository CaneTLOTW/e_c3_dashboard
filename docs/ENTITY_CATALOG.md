...
| **Trailing consumption (500 km)**  `sensor.<vehicle>_dashboard_trailing_consumption_500_km` | Rolling consumption over the most recent locally completed trips, capped at 500 km. On first installation it also imports suitable historical native **Last trip** rows from Recorder. The sensor attributes report covered distance, contributing energy, trip count, and whether the full 500 km are available. | **Medium to low.** Distance is native upstream data; energy is estimated from SOC decrease × reported battery capacity (or 43.4 kWh fallback). Historic rows are accepted only when the native trip includes distance, start mileage, duration and usable SOC at its boundaries. It excludes charging losses and may be quantised because upstream SOC can be whole percentages. Before 500 km of qualifying data exist, it is a partial-window value; check `complete: false`. |
