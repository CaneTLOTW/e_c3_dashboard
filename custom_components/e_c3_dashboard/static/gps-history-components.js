...
if (!window.customCards.some((card) => card.type === DATE_CARD_TAG)) {
...
if (!window.customCards.some((card) => card.type === MAP_CARD_TAG)) {
  window.customCards.push({
    type: MAP_CARD_TAG,
    name: "e-C3 GPS History Map",
    description: "Date-filtered e-C3 GPS and server trip history map",
  });
}
