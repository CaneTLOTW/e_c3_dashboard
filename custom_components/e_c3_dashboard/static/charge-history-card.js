...
class CodexStellantisChargeCurveCardV1 extends LitElement {
    ...
    _chart(points, type) {
        if (points.length < 2) return html`<p class="muted">Noch keine zwei verwertbaren SOC-Meldungen für die Kurve.</p>`;
        const width = 720;
        const height = 235;
        const left = 48;
        const right = 16;
        ...
        const mid = yMax / 2;
        return html`
            <svg class="chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="Ladeleistung über SOC">
    ...
    render() {
        ...
        return html`
            <ha-card>
                <div class="card-content">
                    <div class="header">
                        <span class="title">${this._config.title} · ${type}</span>
                        <span class="status ${curve?.active ? "live" : ""}">${curve?.active ? "● lädt" : "letzter Ladevorgang"}</span>
                    </div>
                    ${this._loading && !curve ? html`<p class="muted">Ladekurve wird geladen …</p>` : nothing}
                    ${this._error ? html`<p class="error">Ladekurve konnte nicht geladen werden: ${this._error}</p>` : nothing}
                    ${curve ? html`
                        <div class="metrics">
                            ...
                            <div class="metric"><div class="metric-label">Dauer</div><div class="metric-value">${this._duration(durationSeconds)}</div></div>
                            <div class="metric"><div class="metric-label">Energie</div><div class="metric-value">${this._number(energy, 2)} kWh</div></div>
                            <div class="metric"><div class="metric-label">Ø Leistung</div><div class="metric-value">${this._number(average, 1)} kW</div></div>
                        </div>
                        ${this._chart(curve.points, type)}
                        <span class="hint">Leistung je SOC-Schritt, batterie­seitig aus SOC und Zeit abgeleitet. Die Kurve bleibt bis zum nächsten bestätigten Fahrtbeginn sichtbar.</span>
                    ` : nothing}
                </div>
            </ha-card>`;
    }
...
class CodexStellantisChargeCurveBrowserCardV1 extends LitElement {
    ...
    _formatSession(session) {
        const date = new Date(session.start).toLocaleString("de-DE", { dateStyle: "short", timeStyle: "short" });
        return `${date} · ${session.charge_type} · ${this._number(session.soc_start, 0)} → ${this._number(session.soc_end, 0)} %`;
    }

    _chart(points, type) {
        if (points.length < 2) return html`<p class="muted">Für diesen Ladevorgang liegen nicht genug SOC-Schritte vor.</p>`;
        const width = 720;
        const height = 235;
        const left = 48;
        const right = 16;
        ...
        const path = points.map((point, index) => `${index === 0 ? "M" : "L"}${x(point.soc).toFixed(1)},${y(point.power_kw).toFixed(1)}`).join(" ");
        const color = type === "DC" ? "var(--success-color)" : "var(--primary-color)";
        return html`<svg class="chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="Ladeleistung über SOC">
            <line class="grid" x1="${left}" y1="${y(0)}" x2="${width - right}" y2="${y(0)}"></line>
    ...
    render() {
        ...
        return html`<ha-card>
            <div class="card-content">
                <div class="header"><span class="title">${this._config.title}</span><span class="muted">${sessions.length} Vorgänge</span></div>
                ${this._loading && !sessions.length ? html`<p class="muted">Ladevorgänge werden geladen …</p>` : nothing}
                ${this._error ? html`<p class="error">Historie konnte nicht geladen werden: ${this._error}</p>` : nothing}
                ${sessions.length ? html`
                    <select aria-label="Ladevorgang auswählen" .value=${this._selectedId ?? ""} @change=${this._selectSession}>
                    ...
                    ${selected ? html`
                        <div class="metrics">
                            ...
                            <div class="metric"><div class="metric-label">Dauer</div><div class="metric-value">${this._duration(selected.duration_seconds)}</div></div>
                            <div class="metric"><div class="metric-label">Energie</div><div class="metric-value">${this._number(selected.energy_kwh, 2)} kWh</div></div>
                            <div class="metric"><div class="metric-label">Ø Leistung</div><div class="metric-value">${this._number(selected.average_power_kw, 1)} kW</div></div>
                        </div>
                        ${this._chart(curve?.points ?? [], type)}
                        <span class="hint">${type} · batterie­seitig aus SOC und Zeit abgeleitet. Verfügbar im Recorder-Zeitraum.</span>
                    ` : nothing}
                ` : !this._loading && !this._error ? html`<p class="muted">Keine abgeschlossenen Ladevorgänge im gewählten Zeitraum.</p>` : nothing}
            </div>
        </ha-card>`;
    }
}
