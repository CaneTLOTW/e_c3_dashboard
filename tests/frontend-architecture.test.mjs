...
test("Home Assistant registers one e-C3 frontend resource", () => {
  ...
  assert.match(frontend, /import\("\.\/vehicle-overview-card\.js\?v=0\.5\.43"\)/);
  assert.match(frontend, /import\("\.\/gps-history-card\.js\?v=0\.5\.43"\)/);
...
test("dependency preflight waits instead of failing on first customElements lookup", () => {
  ...
  assert.match(frontend, /await import\("\.\/e_c3_dashboard\.js\?v=0\.5\.43"\)/);
});
